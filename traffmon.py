#!/usr/bin/python3
from config import COUNTRIES, TRAFFMON_DEBUG, NFSEN_URL, BAD_AS_LIST
...
def floor_time(dt=None, round_to=60):
	if dt == None: 
	...
	timestamp = int(datetime.datetime.timestamp(dt + datetime.timedelta(0,rounding-seconds,-dt.microsecond)))
...
def adjust_timestamp(timestamp):
	...
	if DEBUG:
		print(" IN timestamp:", nfsenutils.to_nfsen_time(timestamp))

	if timestamp >= int(time.time()) - 60*4:
		if DEBUG:
			print("Timestamp too recent, going back in time for safety.")

		timestamp = floor_time(timestamp, 60*5) - 5*60
	else:
		timestamp = floor_time(timestamp, 60*5)
	
	if DEBUG:
		print("OUT timestamp:", nfsenutils.to_nfsen_time(timestamp))
