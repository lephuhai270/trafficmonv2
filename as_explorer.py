from config import COUNTRIES, AS_EXPLORER_DEBUG, NFSEN_URL, AS_EXPLORER_CACHE_EXPIRATION_INTERVAL, BAD_AS_LIST
from traffmon import adjust_timestamp

DEBUG = AS_EXPLORER_DEBUG
NFSEN_SERVER = NFSEN_URL
...
def _find_limit(entries):
	...
	ignore_list = [ipaddress.IPv4Address(entry[nfsenutils.IP]) for entry in entries[first_appearance:] if ":" not in entry[nfsenutils.IP]]
...
def as_explorer(interface, router, AS_CAPTURE=None, timestamp=None, silent=False):
	...
	if timestamp is None:
		timestamp = time.time()
	timestamp = adjust_timestamp(timestamp)

	nfsen = nfsenutils.NFSENServer(NFSEN_SERVER)
	...
	if has_as_zero:
		...
		for a in range(3):
			...
			nfsen_dump = nfsenutils.get_sdump_entries(nfsen_output)
			whoip.whoipe(nfsen_dump, AS_EXPLORER_CACHE_EXPIRATION_INTERVAL)

			networks = load_cache()

			for entry in nfsen_dump:
				try:
					# IPv6 are truncated in HTML output, therefore will never
					# be able to parse it.
					ip = ipaddress.IPv4Address(entry[nfsenutils.IP])
				except ipaddress.AddressValueError:
					print("INVALID IPv4: %s. Skipping." %ip)
					continue
				...
				pass_info["nfsen_bw"] += bps
				pass_info["calc_bw"] += calc_bps
...
#=====================================================
	if has_as_zero:
		...
		nfsen_bw_total = 0
		calc_bw_total = 0
		for pass_i in passes_info:
			nfsen_bw_total += pass_i["nfsen_bw"]
			calc_bw_total += pass_i["calc_bw"]

		print("       calc, nfsen")
		for i, pass_i in enumerate(passes_info, 1):
			nfsen_percent = round((pass_i["nfsen_bw"] / nfsen_bw_total) * 100,2)
			calc_percent = round((pass_i["calc_bw"] / calc_bw_total) * 100,2)
			print("PASS %i:   %s%%,  %s%%" % (i,calc_percent, nfsen_percent))
#=====================================================
	if silent:
