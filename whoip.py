...
from get_subnets import PRIVATE_NETWORKS

CACHE = "network_cache"


def whois(ip):
	ip_o = ipaddress.IPv4Address(ip)
	for private_net in PRIVATE_NETWORKS:
		if ip_o in private_net:
			return None, None
...
def whoipe(nfsen_dump, cache_expiry=60 * 60 * 24):
	...
	#Expire cache every 24h mandatory
	if cache["time"] + cache_expiry < time():
		renew_cache = True
	else:
